...
export default function CaptureContainer(capture_data=null) {
    ...
    let setCaptureIndex = (value)=>dispatch({type:"CHANGE_CAPTURE_INDEX",data:value})
    ...
    const changePage = (e) => {
        e.preventDefault()
            ...
            if( capture_index_field > capture_data.length){
                setIndexInvalid(1)
                setMessage(`Existe(m) apenas ${capture_data.length} captura(s) cadastrada(s)`)
            }else{
                    ...
                    <nav aria-label="Page navigation example">
                        <ul className="pagination justify-content-center">
                            <li className="page-item">
                                <button className="page-link btn" href="#" disabled={capture_index === 0} onClick={() => { setCaptureIndex(capture_index - 1); setCaptureIndexField(capture_index) }} tabindex="-1"><span>&#60;</span></button>
                            </li>
                            <div className="container m-0 p-0" style={{ width: "3rem" }}>
                                <form onSubmit={(e) => {
                                    changePage(e)
                                }}>
                                    <li className="page-item" style={{ width: "inherit" }}><input className={`page-link form-control ${is_invalid_class} `}
                                        onChange={(e) => setCaptureIndexField(e.target.value)}
                                        value={capture_index_field} style={{ width: "3rem" }} href="#"></input>
                            ...
                            <li className="page-item">
                                <button className="page-link btn" disabled={capture_index === (capture_data.length - 1)} onClick={(e) => { setCaptureIndex(capture_index + 1); setCaptureIndexField(capture_index + 2) }} href="#">></button>
                            </li>
                        </ul>
                    </nav>
