...
export default function CaptureInit() {
    ...
    let band_sizes = useSelector(state => state.initial_data.bands.map((string) => string.size).filter(onlyUnique))

    let band_options = useSelector(state => state.initial_data.bands.bands)
    console.log(band_options)
    let capture_codes = acc_code.filter((value) => value.name === "band_code")[0].options.map((vals) => vals.value_oama)
